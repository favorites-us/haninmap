...
**조건:** 카테고리가 의료/치과 계열이고, HaninDoc 지원 도시일 때
...
HaninMap의 `city` 필드 (lowercase)를 HaninDoc 도시 slug에 매핑:

```ts
const HANINMAP_TO_HANINDOC_CITY: Record<string, string> = {
...
**저장 방식:** `GooglePlace.sourceId`에 이미 HaninDoc UUID가 있는 업체가 있으므로, 새로운 nullable 필드 `Business.hanindocSlug String?`을 추가하여 slug를 저장한다.
